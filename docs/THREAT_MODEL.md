...
| Relayer abuse              | Wallet signature, one-time challenge, daily quota       | Public endpoints still need hosting-level rate limits  |
| Worker crash               | PostgreSQL recovery plus idempotent chain state         | Submission can require an event lookup after restart   |
