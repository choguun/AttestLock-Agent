...
September 9 Chrome/Rabby approval, lock, signed queue, refresh recovery and second native line are documented in [LIVE_UI_AUDIT.md](LIVE_UI_AUDIT.md) and its sanitized fixture. Previously claimed faucet tokens were used; the new line was not drawn. Pending: actual seven-day repayment on the original line, final-release scheduled smoke, operational crash/replacement drills and final live-demo/uncut videos. Ingress client-IP semantics remain unverified; no arbitrary forwarded headers are trusted. The full evidence checker intentionally still fails without the complete receipt set, especially post-maturity repayment. Use the executed fixture as `PROOF_FIXTURE_FILE` and the original fixture as `TAMPER_PROOF_FIXTURE_FILE` when rerunning it.
